...
from Multi_GCN import MultiHeadAttention, MultiGraphConvLayer
...
class DocREModel(nn.Module):
    ...
    def __init__(self, config, model, tokenizer, args,
        ...
        self.module_sequence_selector = args.module_sequence_selector

        self.Graph_layers = nn.ModuleList()
        if self.module_sequence_selector == 0:
            self.Graph_layers.append(Transformer(self.emb_size))
            for i in range(self.num_layers-1):
                layers = nn.ModuleList()
                layers.append(MultiHeadAttention(self.heads, self.emb_size))
                layers.append(MultiGraphConvLayer(args, self.emb_size, 4, self.heads))
                self.Graph_layers.append(layers)
        else:
            for i in range(self.num_layers - 1):
                layers = nn.ModuleList()
                layers.append(MultiHeadAttention(self.heads, self.emb_size))
    ...
    def forward_rel(self, hs, ts, rs, input_ids):
        '''
        ...
        # ts = ts.unsqueeze(0)
        #
        # input_ids = input_ids[:, :hs.size(1)]
        # input_ids.view(input_ids.size(0), hs.size(1))
        #
        # print("input_ids:shape", input_ids.shape)
        # print("ts:shape", ts.shape)
        # transformer = Transformer(self.emb_size)
        # # print("CPU Tensor Device:", hs.device)
        # # # input_ids = input_ids.to('cpu')
        # # # hs = hs.to('cpu')
        # # print("CPU Tensor Device:", input_ids.device)
        # hs_t, _ = transformer(hs, input_ids)
        # ts_t, _ = transformer(ts, input_ids)
        # hs_t = hs_t.squeeze(0)
        # ts_t = ts_t.squeeze(0)
        # print("transformer:shape", hs_t.shape)
        # print("transformer:shape", ts_t.shape)
        # hs = hs.squeeze(0)
        # ts = ts.squeeze(0)
        ...
        # print("b1.unsqueeze(3): ", b1.unsqueeze(3).shape)
        # print("b2.unsqueeze(2): ", b2.unsqueeze(2).shape)
        # print("(b1.unsqueeze(3) * b2.unsqueeze(2)).shape: ", (b1.unsqueeze(3) * b2.unsqueeze(2)).shape)
    ...
    def forward(self,
        ...
        if self.module_sequence_selector == 0:
            ...
            transformer = self.Graph_layers[0]
            # input_ids: (batch_size, doc_len)
            # enc_outputs: (batch_size, doc_len, hidden_dim)
            # sequence_output: (batch_size, doc_len, hidden_dim)
            # attention: (batch_size, num_attn_heads, doc_len, doc_len)
            enc_outputs, enc_self_attns, sequence_output = transformer(sequence_output, input_ids)
            attention = attention + enc_self_attns / 2

            # gcn
            PAD_ID = 0
            src_mask = (input_ids != PAD_ID).unsqueeze(-2)
            for i in range(1, self.num_layers):
                attn_tensor = self.Graph_layers[i][0](sequence_output, sequence_output, src_mask)
                attn_adj_list = [attn_adj.squeeze(1) for attn_adj in torch.split(attn_tensor, 1, dim=1)]
                sequence_output = self.Graph_layers[i][1](attn_adj_list, sequence_output)  # mem_dim
                print(i)
        else:
            # gcn
            PAD_ID = 0
            ...
            transformer = self.Graph_layers[self.num_layers-1]
            # input_ids: (batch_size, doc_len)
            # enc_outputs: (batch_size, doc_len, hidden_dim)
            # sequence_output: (batch_size, doc_len, hidden_dim)
            # attention: (batch_size, num_attn_heads, doc_len, doc_len)
        ...
        logits = self.forward_rel(hs, ts, rs, input_ids)
        # print(1 / 0)

...
from model_gcn import DocREModel
from utils import set_seed, collate_fn, create_directory
from prepro import read_docred
from evaluation_revistdocred import to_official, official_evaluate, merge_results
import wandb
...
def train(args, model, train_features, dev_features):
    ...
    def finetune(features, optimizer, num_epoch, num_steps):
        ...
        for epoch in tqdm(train_iterator, desc='Train epoch'):
            for step, batch in enumerate(train_dataloader):
                ...
                if inputs["sent_labels"] != None:
                    loss_evi = outputs["loss"]["evi_loss"] * args.evi_lambda
                    loss.append(loss_evi)
                    loss_list_evi.append(loss_evi.item())
                if inputs["teacher_attns"] != None:
                    loss_rel = outputs["loss"]["attn_loss"] * args.attn_lambda
                    loss.append(loss_rel)
                    loss_list_rel.append(loss_rel.item())
        ...
        print("loss_list_rel:{}".format(loss_list_rel))
        print("loss_list_evi:{}".format(loss_list_evi))
        with open("loss_list.txt", "w",encoding='utf-8') as file1:
            for item in loss_list:
                file1.write(item + "\n")

        with open("loss_list_rel.txt", "w",encoding='utf-8') as file2:
            for item in loss_list_rel:
                file2.write(item + "\n")

        with open("loss_list_evi.txt", "w",encoding='utf-8') as file3:
            for item in loss_list_evi:
                file3.write(item + "\n")
        return num_steps
...
def main():
    ...
    formatted_datetime = current_datetime.strftime("%Y-%m-%d_%H:%M:%S").replace(":", "_")
    save_path_ = os.path.join(args.save_path, f"{formatted_datetime}")
    ...
    model = DocREModel(config, model, tokenizer,args,

    ...
    if c <= 512:
        ...
        for (n_s, l_i) in zip(num_seg, seq_len):
            if n_s == 1: # 1 segment (no split)
                ...
                output = (output1 + output2) / mask.unsqueeze(-1)  # (doc_len, hidden_dim)
                att = (att1 + att2)  # (model_head, doc_len, doc_len)
        ...
        attention = torch.stack(new_attention, dim=0)  # (bath_size model_head, doc_len, doc_len)
